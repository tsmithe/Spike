...
** TODO As few naked pointers as possible (use references)
** TODO Use proper units for time (and possibly other quantities?..)
** TODO Electrodes write out buffer info too early
** TODO Buffer interval cannot be set with float if dt isn't already set
