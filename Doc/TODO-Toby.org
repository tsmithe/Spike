...
:END:
**** TODO RateSynapses::set_delay sets storage size for corresponding presynaptic RateNeurons
...
**** TODO Sparse Hebb update: use element_prod to zero unconnected inputs (before doing the outer_prod!)
