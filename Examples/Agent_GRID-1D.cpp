...
int main(int argc, char *argv[]) {
  ...
  */
  ...
  agent.set_place_test_params(0.q*radius, 20);
  ...
  FloatT PLACE_PLACExFVxHD_scaling = 500.0 / (N_PLACE*PLACE_PLACExFVxHD_sparsity);
  ...
  FloatT VIS_PLACE_scaling = 500.0 / (N_PLACE*VIS_PLACE_sparsity);
  FloatT VIS_PLACE_INH_scaling = -1.0 / (N_VIS*VIS_PLACE_sparsity);
  FloatT eps_VIS_PLACE = 0.06;

  // PLACExFVxHD -> PLACE connectivity:
  FloatT PLACExFVxHD_PLACE_scaling = 6000.0 / N_PLACExFVxHD;

  // PLACE -> PLACE connectivity:
  FloatT PLACE_inhibition = -750.0 / N_PLACE;
